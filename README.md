...
1) Installieren von externen Modulen
	...
	6) Der Server - server.js
		...
		Die response soll aus dem index.jade file gerendert werden, welches wir noch erstellen. Da wir oben den View Engine auf Jade gesetzt haben weiß der Server, dass er mit render ein .jade File rendern soll. Es wird im Endeffekt ein einfaches html Dokument an den Nutzer gesendet. Allerdings können wir dem Renderengine noch alle Skripte und stylesheets und den Titel übergeben.
		...
		Am Ende returnen wir noch den Server und voilá das Skellett unseres Frontends steht.
	...
	7) Das wirkliche Front End - index.jade
		...
		Dies ist unser Template, das der Server rendert und dann als response an den Besucher schickt. Hier definieren wir also, was wir dem Besucher tatsächlich zeigen.
		...
		Interessant sind nur die letzten paar Zeilen. Damit die Graphen richtig angezeigt werden können müssen wir für jeden Graphen einen Canvas Objekt erstellen. Mit "#....." definieren wir die html ID des Canvas. Darauf können wir dann im Skript, das die Daten in die Graphen lädt, zugreifen.
		Mit dem 
			- each s in scripts
			      script(src=s)
		Importieren wir alle js Dateien die wir im Server der Variablen script zugewiesen haben. Und zu diesen beiden kommen wir jetzt auch schon.
...
8) Die Graphen - chartmaker.js
		...
		Zuerst müssen wir jetzt Chart.bundle.min.js von gitHub ( https://github.com/chartjs/Chart.js/releases/tag/v2.3.0 )herunterladen und in diesem Verzeichnis (JS) speichern. 
		...
		Diese soll periodisch ab dem Laden der Seite über unseren "/weather" Socket vom Server die Daten des Sensors abrufen und darstellen. Hierzu definieren wir zuerst eine request Methode, die eine request an den Server schickt.
		...
		Wenn wir uns erinnern - bei request an "/weather" gibt der Server uns folgendes zurück:
			buffer.value = {
	          temperature: filteredData.map(transform.temperature),
	          humidity: filteredData.map(transform.humidity),
	          pressure: filteredData.map(transform.pressure),
		...
		Wir rufen direkt das grade definierte request auf mit dem Ziel "/weather" und definieren eine anonyme Funktion als Callback. Diese bekommt die Antwort (obj) als Argument und kann dann damit arbeiten (also per obj.party bekommen wir den Datensatz der Partyindexe).
		...
		Wichtig ist den kaskadenartigen Aufbau zu verstehen und sich nicht von den ganzen Details ablenken zu lassen. Im Grunde ist ein Chart so aufgebaut:
			new Chart(....){
				type: 'line', //oder irgend ein anderer Type
				data: .... , //Unsere Daten
				options: .... //Wie die Achsen aussehen sollen und wie der Graph sich verhält
			}
		Wichtig ist, dass man alle Attribute durch Kommata trennt.
		...
		Wenn wir in dieser Weise alle Graphen innerhalb von refreshChart() definiert haben (sh. source Code hier auf github) benutzen wir wieder ein setInterval() um die Graphen jede Minute zu refreshen:
